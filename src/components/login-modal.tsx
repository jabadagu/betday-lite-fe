        ...
        <>
          ...
          >
            <Card className="w-full max-w-md" onClick={(e) => e.stopPropagation()}>
              ...
              <Typography variant="body2" className="mt-1 text-sm text-tertiary">
                {copy.loginSubtitle}
              </Typography>

              <form className="mt-4 space-y-3" onSubmit={onSubmit}>
                <Box className="space-y-1.5">
                  <label className="text-xs font-semibold text-secondary">{copy.email}</label>
                  <Input placeholder="ana@betday.dev" type="email" {...form.register("email")} />
                </Box>
                <Box className="space-y-1.5">
                  <label className="text-xs font-semibold text-secondary">{copy.password}</label>
